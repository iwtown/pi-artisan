...
skill 全生命周期：/find-skills 搜索 → /skill-vetter 安全审查 → skill_acquire 安装 → /create-skill scaffold → /adapt 适配检查 → /resource-birth 出生证 → 编辑 SKILL.md → /validate-skill 校验。
常用工具：/create-skill, /adapt, /resource-birth, /validate-skill, /optimize-skill, /resource-publish
安装外部来源的 skill 请用 skill_acquire 工具。`,
  extension: `🧰 pi-artisan 路由：检测到 extension 操作。
extension 全生命周期：/find-skills 搜索 → /create-extension scaffold → /adapt 适配检查 → /resource-birth 出生证。
常用工具：/create-extension, /adapt, /validate-extension, /resource-birth`,
  prompt: `🧰 pi-artisan 路由：检测到 prompt 操作。
prompt 全生命周期：/find-skills 搜索 → /create-prompt scaffold → /adapt 适配检查 → /resource-birth 出生证。
常用工具：/create-prompt, /adapt, /validate-prompt, /resource-birth`,
  theme: `🧰 pi-artisan 路由：检测到 theme 操作。
theme 全生命周期：/find-skills 搜索 → /create-theme scaffold → /adapt 适配检查 → /resource-birth 出生证。
常用工具：/create-theme, /adapt, /validate-theme, /resource-birth`,
  package: `🧰 pi-artisan 路由：检测到 package 操作。
package 全生命周期：/find-skills 搜索 → /create-package scaffold → /adapt 适配检查 → /resource-birth 出生证。
常用工具：/create-package, /adapt, /validate-package, /resource-birth`,
};

/**
 * 生成路由提示，根据生命周期技能安装状态动态调整。
 */
function buildRoutingHint(typeId: string): string {
  const base = ROUTING_HINTS[typeId];
  if (!base) return "";

  const skills = checkLifecycleSkills();
  const relevant = skills.filter((s) => s.appliesTo.includes(typeId));
  if (relevant.length === 0) return base;

  const statusLines = relevant.map((s) =>
    s.installed
      ? `   ✅ ${s.name} 已就绪 — ${s.hint}`
      : `   ⚠️ ${s.name} 未安装 — 可通过 /find-skills 搜索安装或从 pi-capabilities 添加`
  );
...
export function setupInputHook(pi: ExtensionAPI): void {
  pi.on("input", async (event, _ctx) => {
    ...
    const hint = buildRoutingHint(routing.typeId);
